...
def test_authz(token_config: Dict[str, Any], rbac_config: Dict[str, Any]) -> None:
    ...
    random_user = secrets.SystemRandom().choice([user for user in authz_config["users"] if user["id"] != "__master__"]
    )
    _master_user = [
        user for user in authz_config["users"] if user["id"] == "__master__"
    ][0]
    random_token = secrets.SystemRandom().choice(random_user["tokens"])["token"]
